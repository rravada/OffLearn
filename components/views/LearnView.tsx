...
export function LearnView({ curriculum, selectedSubject }: LearnViewProps) {
  ...
  const { setTutorOpen, setTutorSystemPrompt, clearTutorMessages } = useAppStore();
                  ...
                  <div className="space-y-2">
                    {unit.lessons.map((lesson, lessonIdx) => (
                      <button
                        ...
                        className="group flex w-full items-center gap-4 rounded-xl border border-le-border bg-le-surface px-5 py-4 text-left transition-all hover:border-le-border-strong hover:bg-le-elevated"
                      >
